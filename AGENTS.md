...
- A tenant can be created rather than declared. `apps/hub/src/host/fly-provision.ts` (`npm run machine -- create <app> <org> <image> [region]`) makes the app, the volume and the Machine; `COMPUTER_CATALOG` on the control plane (`id|hubUrl|label|setupCodeEnv`, comma separated, last two optional) is what stops that needing a code change. Unset, the catalog is still the seeded Blode/Vibey pair; setting it replaces the pair entirely. Provisioning sets **no** secrets and refuses to be asked to: Fly delivers app secrets as environment variables, so a setup code in `config.env` would boot fine and also read back out of `GET /machines/<id>`. Set it with `fly secrets set -a <app>` first.
