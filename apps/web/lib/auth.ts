...
const staticTrustedOrigins = [...new Set([canonicalOrigin, siteConfig.url, `https://www.${new URL(siteConfig.url).host}`])];

// Local dev: reflect any localhost origin so the portless / next port is trusted.
const trustedOrigins = (request?: Request): string[] => {
  if (isProduction || !request) return staticTrustedOrigins;
...
if (allowedEmails.size === 0 && isProductionRuntime) {
  console.warn("[auth] AUTH_ALLOWED_EMAILS is unset: anyone who can receive email can sign in and own the computer");
}
      ...
      create: {
        before: async (user) => {
          if (allowedEmails.size > 0 && !allowedEmails.has(user.email.toLowerCase())) {
            throw new APIError("FORBIDDEN", { message: "This computer is private. Ask its owner for access." });
          }
          return { data: user };
        },
      },
  ...
  plugins: [
    emailOTP({
      sendVerificationOTP: async ({ email, otp, type }) => {
        if (allowedEmails.size > 0 && !allowedEmails.has(email.toLowerCase())) return;
