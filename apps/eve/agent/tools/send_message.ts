...
/**
 * The voice. Everything else Eve writes is a private scratchpad — this is
 * the only call the human ever sees, so a turn that ends without one is
 * silence and the app looks frozen.
 *
 * Flat object rather than a discriminated union, to match the wire shape in
 * api/spec.json exactly; the hub validates the per-kind fields.
 */
