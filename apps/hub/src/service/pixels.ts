...
const DEFAULT_PIXEL_TTL_MS = 15 * 60 * 1000;
...
export class PixelRegistry {
  ...
  mint(display: number, now = Date.now()): PixelGrant {
    ...
    const grant: PixelGrant = { display, expires: now + this.ttlMs, token };
  ...
  grantFor(display: number, now = Date.now()): PixelGrant {
    ...
    for (const grant of this.grants.values()) {
      if (grant.display !== display) {
        continue;
      }
      if (grant.expires - now <= PIXEL_REFRESH_MS) {
        continue;
      }
      if (!best || grant.expires > best.expires) {
        best = grant;
      }
    }
  ...
  private writeForkFile(display: number, token: string): void {
    if (!this.tokenDir || display < 2) {
      return;
    }
    mkdirSync(this.tokenDir, { mode: 0o700, recursive: true });
...
export function withPixelToken(base: string, grant: PixelGrant): string {
  try {
    ...
    if (grant.display > 1) {
      u.searchParams.set("display", String(grant.display));
    }
    return u.toString();
  } catch {
