...
export class VoiceService {
  ...
  restore(entries: Occurrence[]): void {
    ...
    this.seq = this.log[this.log.length - 1]?.seq ?? 0;
    ...
    if (last?.kind === "widget" && last.answer === null) this.turnEnded = true;
  ...
  private persist(o: Occurrence): void {
    if (!this.transcript || !this.restored) return;
    this.writes = this.writes
      .then(() => this.transcript!.appendOccurrence(o))
      .catch((err: unknown) => {
        console.warn(`transcript: ${(err as Error).message}`);
      });
  }
