...
import { ComputerError, EVE_HUB_SECRET_HEADER, outboundReply } from "@computer/shared";
...
/**
 * How long the hub waits on a Bot's Eve for one turn, on every path that
 * forwards a message to it. Long, because an owner's scheduled work is a
 * turn too. Finite, because an Eve conversation whose first turn failed
 * terminally never starts another run (seen on vcmc-computer, 2026-09-06),
 * and without a deadline the hub held that request open until the bridge
 * gave up, with no error anyone could read.
 */
export const EVE_TURN_TIMEOUT_MS = 30 * 60_000;
...
export async function runEveTurn(input: {
  ...
  try {
    const upstream = await fetch(input.url, {
      ...
      method: "POST",
      redirect: "manual",
      signal: AbortSignal.timeout(EVE_TURN_TIMEOUT_MS),
    });
