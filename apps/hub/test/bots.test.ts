...
async function startTwoBots() {
  ...
  const h = await startHub({
    bots: [
      { display: 1, id: "a", token: "token-a" },
      { display: 2, id: "b", token: "token-b" },
    ],
    desks: new Map([
      [1, deskA],
      [2, deskB],
...
describe("bots: one shared box, one screen per Bot", () => {
  ...
  it("routes each agent token to its own screen", async () => {
    ...
    await rpc(
      ...
      { actions: [{ type: "click", x: 10, y: 20 }], request_id: "r1" },
      "token-a",
    );
    await rpc(
      h.url,
      "/computer.v1.Agent/Computer",
      { actions: [{ type: "click", x: 30, y: 40 }], request_id: "r2" },
      "token-b",
    );
  ...
  it("rejects an unknown agent token", async () => {
    const h = await startTwoBots();
    opened.push(h);
    await expect(rpc(h.url, "/computer.v1.Agent/Spec", {}, "token-c")).rejects.toMatchObject({
  ...
  it("SEAT_HELD is per screen: takeover on a leaves b runnable", async () => {
    ...
    await rpc(
      h.url,
      "/computer.v1.Agent/Computer",
      { actions: [{ type: "request_takeover" }], request_id: "t1" },
      "token-a",
    );
    await expect(
      rpc(h.url, "/computer.v1.Agent/Shell", { argv: ["echo", "hi"], request_id: "s1" }, "token-a"),
    ).rejects.toMatchObject({ code: "SEAT_HELD" });

    const r = (await rpc(
      h.url,
      "/computer.v1.Agent/Shell",
      { argv: ["echo", "hi"], request_id: "s2" },
      "token-b",
    )) as { exit: number };
  ...
  it("SetPresence with display releases only that screen", async () => {
    ...
    await rpc(
      h.url,
      "/computer.v1.Agent/Computer",
      { actions: [{ type: "request_takeover" }], request_id: "t2" },
      "token-b",
    );
  ...
  it("Seat display outside 1..8 or unassigned is VALIDATION", async () => {
    ...
    const token = await h.pair();
    await expect(
      rpc(h.url, "/computer.v1.Seat/Status", { display: 9 }, token),
    ).rejects.toMatchObject({
      code: "VALIDATION",
    });
    await expect(
      rpc(h.url, "/computer.v1.Seat/Status", { display: 3 }, token),
    ).rejects.toMatchObject({
      code: "VALIDATION",
    });
  });
  ...
  it("Pointer and Type route by display", async () => {
    ...
    await rpc(
      ...
      { display: 2, dx: 5, dy: 5, type: "move" },
      token,
    );
    await rpc(h.url, "/computer.v1.Seat/Type", { display: 2, text: "hello" }, token);
    ...
    await expect(
      rpc(h.url, "/computer.v1.Seat/Pointer", { dx: 1, dy: 1, type: "move" }, token),
    ).rejects.toMatchObject({ code: "SEAT_HELD" });
  });
  ...
  it("registry rejects duplicates, bad ids, and out-of-range displays", () => {
    ...
    expect(() =>
      make([
        { display: 1, id: "a", token: "t1" },
        { display: 2, id: "a", token: "t2" },
      ]),
    ).toThrow(/already exists/);
    expect(() =>
      make([
        { display: 1, id: "a", token: "t1" },
        { display: 1, id: "b", token: "t2" },
      ]),
...
describe("provisioning: computer as a service", () => {
  ...
  it("DeleteBot releases the screen and revokes the token", async () => {
    ...
    const again = (await rpc(h.url, "/computer.v1.Seat/CreateBot", { id: "day" }, token)) as {
      display: number;
    };
  ...
  it("refuses duplicates, bad ids, deleting the primary, and requires a seat token", async () => {
    ...
    const token = await h.pair();
    await expect(
      rpc(h.url, "/computer.v1.Seat/CreateBot", { id: "main" }, token),
    ).rejects.toMatchObject({
      code: "CONFLICT",
    });
    await expect(
      rpc(h.url, "/computer.v1.Seat/CreateBot", { id: "Bad Name!" }, token),
    ).rejects.toMatchObject({
      code: "VALIDATION",
    });
    await expect(
      rpc(h.url, "/computer.v1.Seat/DeleteBot", { id: "main" }, token),
    ).rejects.toMatchObject({
      code: "VALIDATION",
    });
    await expect(
      rpc(h.url, "/computer.v1.Seat/CreateBot", { id: "x" }, h.agent),
    ).rejects.toMatchObject({
      code: "UNAUTHENTICATED",
    });
  });
  ...
  it("caps the box at 8 screens with a CONFLICT that names the fix", async () => {
    ...
    await expect(
      rpc(h.url, "/computer.v1.Seat/CreateBot", { id: "one-more" }, token),
    ).rejects.toMatchObject({
      code: "CONFLICT",
  ...
  it("a failed window claim rolls the bot back", async () => {
    ...
    await expect(
      rpc(h.url, "/computer.v1.Seat/CreateBot", { id: "night" }, token),
    ).rejects.toMatchObject({
      code: "DAEMON_DOWN",
    });
...
describe("FileBotStore: the roster is the only record of every bot token", () => {
  ...
  it("throws on a corrupt roster instead of reading it as an empty box", () => {
    ...
    expect(() => store.load()).toThrow(new RegExp(path.replaceAll(/[.*+?^${}()|[\]\\]/g, "\\$&")));
  ...
  it("round-trips a saved roster", () => {
    ...
    const store = new FileBotStore(path);
    store.save([{ display: 1, id: "main", token: "bot_x" }]);
    expect(store.load()).toEqual([{ display: 1, id: "main", token: "bot_x" }]);
  });
});
...
describe("vnc proxy display routing", () => {
  ...
  it("a pixel token opens only the screen it was minted for", async () => {
    const a = await fakeRfb("rfb-display-1");
    const h = await startHub({
      bots: [
        { display: 1, id: "a", token: "token-a" },
      ...
      vncBasePort: a.port - 1,
    });
    opened.push(h);
    const token = await h.pair();
    const status = (await rpc(h.url, "/computer.v1.Seat/Status", {}, token)) as {
      screens: { display: number; vnc_url: string }[];
    };
    const pixel = (display: number) =>
      new URL(status.screens.find((s) => s.display === display)!.vnc_url).searchParams.get("token");
    const wsBase = `${h.url.replace("http", "ws")}/websockify`;

    expect(await wsFirstMessage(`${wsBase}?token=${pixel(1)}&display=1`)).toBe("rfb-display-1");
    expect(await wsFirstMessage(`${wsBase}?token=${pixel(2)}&display=1`)).toBe(4403);
  });
});
