...
1. **One computer for every account (partial).** hello.expert now binds a session to a seeded computer (`matt` → `mblode-computer`, `vcmc` → `vcmc-computer`) with a per-computer setup code. There is still no self-serve Fly provisioning: extra tenants are a Fly app + volume from this repo, not a Machines API call on sign-in. Every seat on a guest is still a box owner (`CreateBot`, `DeleteBot`, clipboard read, `ProvideSecret`, the Eve thread). `AUTH_ALLOWED_EMAILS` remains the sign-in gate. See the roadmap in `docs/GROK-BOT.md`.
