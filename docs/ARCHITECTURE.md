...
**Tenant tables and self-serve provisioning.** Partly present, and the part that is present has moved. The `computer` and `computer_seat` tables exist in Turso and `ensureComputerCatalog()` mirrors the catalog into them; the catalog itself is now `COMPUTER_CATALOG` (`id|hubUrl|label|setupCodeEnv` per entry) with the seeded `blode` and `vibey` pair as the fallback when it is unset, so adding a tenant is configuration rather than an edit to `apps/web/lib/computers.ts`. `apps/hub/src/host/fly-provision.ts` creates the app, the volume and the Machine, so `fly-machine.ts` is no longer only able to wake something someone else made. What is still absent is the two ends: `computer` remains a cache of an environment variable rather than a table you insert into, and nothing calls `createComputer` on sign-up. _Trigger:_ the first tenant who is not a person Matt knows. At that point the catalog becomes rows, which turns `computersFromEnv`, `boundComputerId`, `accessibleComputers` and both invite paths async at once and should ride alone; and the Fly token needs a home, which is not Vercel, because a credential that can destroy every Machine in the org does not belong in the deployment that already holds every tenant's setup code. Section 2's fail-closed binding had to be in place first, and since [#27](https://github.com/mblode/expert/pull/27) it is: self-serve provisioning on top of a fail-open binding is how one account opens another account's box. See [`plans/gateway.md`](plans/gateway.md).
