...
Since 2026-09-04 it also has the roster the product is really about: eight Bots with their own scopes, marks, instructions, skills and schedules, shipped as directories under `apps/eve/bots` and provisioned at boot rather than by hand ([`docs/BOTS.md`](BOTS.md)). What that changes in the table below is narrower than it sounds. Routines are real per Bot and one of them takes a webhook, so the trigger half of a routine exists; the run history, the test run and the UI do not. Bots still cannot message each other, so the front-of-house Bot routes work by writing a file and telling the human, which is the shape Phase 7 replaces rather than a stand-in for it. And eight Bots on a 2 GB Machine only works because they sleep: a Bot with nothing to do has no Eve process and no screen, and is woken by a message, a webhook, a routine falling due, or a human opening its desk (`docs/BOTS.md`, "What a Bot costs").
