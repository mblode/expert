...
The Expert iOS app was removed on 2026-09-05 and its screenshots on
2026-09-06. References to it below describe the earlier design in prose. The
supported interfaces are now WhatsApp and hello.expert.
...
Since 2026-09-04 it also has the roster the product is really about: eight Bots with their own scopes, marks, instructions, skills and schedules, shipped as directories under `apps/eve/bots` and provisioned at boot rather than by hand ([`docs/BOTS.md`](BOTS.md)). What that changes in the table below is narrower than it sounds. Routines are real (seven of them across six Bots) and one Bot takes a webhook, so the trigger half exists; the run history, the test run and the UI do not. A routine does fire on a suspended box, but only because a second always-on app (`apps/clock`) wakes the Machine for it, where Grok simply keeps the account's VM running. Bots still cannot message each other, so the front-of-house Bot routes work by writing a file and telling the human, which is the shape Phase 7 replaces rather than a stand-in for it. And eight Bots on a 2 GB Machine only works because they sleep: a Bot with nothing to do has no Eve process and no screen. A message, a webhook or a routine falling due wakes the process; opening its desk brings the screen back rather than the process (`docs/BOTS.md`, "What a Bot costs").

| Grok Bot                                                      | Here                                                                                                                                                     | Gap                                                                                                    |
| ------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| One computer per **account**                                  | One computer per **tenant** (seeded `blode` / `vibey` Fly apps); session binds to one hub                                                                | Self-serve Fly Machine per sign-up; seats on a guest are still full owners                             |
...
| Onboarding: tools questionnaire, concept intro, suggested bot | Four cards on first sign-in (three claims and the tools question), stored per account                                                                    | No suggested-bot step, and nothing to provision: the computer is paired by the time the session exists |
...
| Routines with schedule + triggers, run history, test run      | A schedule per Bot in `agent/schedules/`, event triggers through a connector and `lib/channels/webhook.ts`                                               | No UI, no run history, no test run                                                                     |
...
| Multi-agent DMs and group chats                               | Eight Bots with their own scopes; a handoff is a file under `/workspace/handoffs/<bot>/`                                                                 | Bot-to-bot messages, group chats, visible handoffs                                                     |
| Share a Bot as a template                                     | **Yes**: export a Bot's setup, publish it behind a hello.expert link, Add Bot installs it on the reader's own computer (`docs/BOTS.md`, "Sharing a Bot") | An installed routine is declared and paused; the template project compiles no schedules                |
...
| Sleep / wake-on-connect                                       | Fly suspends the Machine when idle; Bots sleep inside it too, and `apps/clock` wakes both for a routine                                                  | Grok's runner is simply always on; here a routine depends on a second app being deployed and up        |
...
**Phase 6, one computer per account.** Seeded tenants are in (`blode`, `vibey`, one Fly app and volume each, session bind + per-computer Pair). Still missing: a Fly Machine per user provisioned on first sign-in (Machines API from the web server), the seat token scoped so it cannot provision, idle suspend and wake-on-request through an always-on edge that can proxy WebSockets and SSE.
